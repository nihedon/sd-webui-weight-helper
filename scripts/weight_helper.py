...
def __initialize():
    global network, network_lora, ui_type
    if network_lora is not None or ui_type is not None:
        return

    try:
        from modules_forge import forge_version
        ui_type = UI_TYPE.NEW_FORGE
    except ImportError:
        ui_type = UI_TYPE.A1111
    ...
    if network_lora is None:
        ui_type = UI_TYPE.Unknown


def __get_lora_on_disk(key):
    if network_lora:
        lora_on_disk = network_lora.available_network_aliases.get(key)
        if not lora_on_disk:
            # sdnext
            lora_on_disk = next((v for v in network_lora.available_network_aliases.values() if v.alias == key), None)
        return lora_on_disk
    return None


def get_metadata(key) -> dict[str, Optional[str]]:
    ...
    lora_on_disk = __get_lora_on_disk(key)
    if lora_on_disk:
        ret["sd_version"] = __get_sd_version(lora_on_disk)

        metadata = lora_on_disk.metadata
        ss_network_args = __get_network_args(lora_on_disk)
        ret["algorithm"] = __get_algorithm(metadata, ss_network_args)
...
def get_preview_info(key):
    ...
    if lora_on_disk:
        ...
        ret["preview_url"] = __find_preview(model_path)
        ret["has_metadata"] = __find_metadata(lora_on_disk)
...
def __get_sd_version(lora) -> Optional[str]:
    if ui_type == UI_TYPE.A1111:
        sd_version: Optional[str] = lora.sd_version.name
        if sd_version and network:
            if sd_version == network.SdVersion.SD1.name or sd_version == network.SdVersion.SD2.name:
                return "SD"
            elif sd_version == network.SdVersion.SDXL.name:
                return "SDXL"
            return sd_version
    elif ui_type == UI_TYPE.NEW_FORGE:
...
def __get_algorithm(metadata, ss_network_args) -> Optional[str]:
...
def __link_preview(filename) -> str:
    quoted_filename = urllib.parse.quote(filename.replace('\\', '/'))
    # use browser cache
    # mtime, _ = lister.mctime(filename)
